...
@shared_task
def purge_expired_deletion_accounts():
    """
    Finds UserAccounts where deletion_requested_at is older than
    settings.ACCOUNT_DELETION_GRACE_PERIOD_DAYS, and hard deletes them from DB and Firebase.
    """
    from django.conf import settings
    ...
    grace_days = getattr(settings, 'ACCOUNT_DELETION_GRACE_PERIOD_DAYS', 7)
    limit = timezone.now() - timedelta(days=grace_days)
    
    expired_accounts = UserAccount.objects.filter(deletion_requested_at__lte=limit)
    logger.info(f"[purge_accounts] Found {expired_accounts.count()} accounts to purge.")

    for account in expired_accounts:
        uid = account.username  # Firebase UID is stored as username
        user = account.user
        
        # 1. Delete from Firebase Auth
        try:
            firebase_auth.delete_user(uid)
            logger.info(f"[purge_accounts] Successfully deleted user {uid} from Firebase Auth.")
        except Exception as fe:
            # If the user doesn't exist on Firebase, we can proceed with DB delete
            logger.warning(f"[purge_accounts] Firebase delete failed/skipped for {uid}: {fe}")

        # 2. Hard delete from Django DB (cascades to UserAccount, Projects, CustomMaterials, Sections)
        try:
            if user:
                user.delete()
                logger.info(f"[purge_accounts] Permanently purged Django User: {user.username} ({user.email})")
            else:
                account.delete()
                logger.info(f"[purge_accounts] Permanently purged orphaned UserAccount: {uid}")
        except Exception as dbe:
            logger.error(f"[purge_accounts] Database deletion failed for user {uid}: {dbe}")
