...
class BeamBeamEndPlateInputData(InputDataBase):
    def process(self, **kwargs):
        email = kwargs.get("email")

        print("Processing Beam-Beam End Plate Input Data - Complete Dataset")

        # Always return ALL data needed for this module
        response = {}

        try:
            # Beam List (no connectivity options for beam-beam connection)
            ...
            print("Beam-Beam End Plate Complete Response: ", {k: len(v) if isinstance(v, list) else v for k, v in response.items()})
            return Response(response, status=status.HTTP_200_OK)

        except Exception as err:
            print(f"Error in BeamBeamEndPlate input handler: {err}")
