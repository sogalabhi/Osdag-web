...
class FinPlateInputData(InputDataBase):
    def process(self, **kwargs):
        ...
        print("Processing Fin Plate Input Data - Complete Dataset")
        ...
        try:
            ...
            print("Fin Plate Complete Response: ", {k: len(v) if isinstance(v, list) else v for k, v in response.items()})
            return Response(response, status=status.HTTP_200_OK)

        except Exception as err:
            print(f"Error in FinPlate input handler: {err}")
