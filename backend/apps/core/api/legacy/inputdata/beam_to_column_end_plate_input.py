...
from apps.core.models import Columns, Beams, Material, CustomMaterials, Bolt

class BeamToColumnEndPlateInputData(InputDataBase):
    def process(self, **kwargs):
        email = kwargs.get("email")

        print("Processing Beam-to-Column End Plate Input Data - Complete Dataset")
        ...
        try:
            ...
            print("Beam-to-Column End Plate Complete Response: ", {k: len(v) if isinstance(v, list) else v for k, v in response.items()})
            return Response(response, status=status.HTTP_200_OK)

        except Exception as err:
            print(f"Error in BeamToColumnEndPlate input handler: {err}")
