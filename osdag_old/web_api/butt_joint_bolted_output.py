...
class ButtJointBoltedOutputView(APIView):
    def post(self, request):
        ...
        if missing_fields:
            print('in butt_joint_bolted_output.py: Missing required fields:', missing_fields)
        try:
            module_api = get_module_api(module_name)
        except Exception as e:
            print('in butt_joint_bolted_output.py: Error getting module API:', e)
            return JsonResponse({"data": {}, "logs": [], "success": False, "error": "Module not found"}, safe=False, status=400)
        output = {}
        logs = []
        new_logs = []
        try:
            print('in butt_joint_bolted_output.py: Trying to generate output')
            output, logs = module_api.generate_output(input_values)
            print('in butt_joint_bolted_output.py: Output generated successfully')
            for log in logs:
                if log not in new_logs:
                    new_logs.append(log)
        except Exception as e:
            print('in butt_joint_bolted_output.py: Exception raised:', e)
            return JsonResponse({"data": {}, "logs": new_logs, "success": False, "error": str(e)}, safe=False, status=400)
        print('in butt_joint_bolted_output.py: Returning final response')
