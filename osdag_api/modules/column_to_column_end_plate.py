...
def create_module() -> ColumnEndPlate:
    """Create an instance of the column column end plate connection module design class and set it up for use"""

    logger.info("Creating ColumnColumnEndPlate module instance")
    try:
        module = ColumnEndPlate()  # Create an instance of the ColumnColumnEndPlate
        logger.debug("ColumnColumnEndPlate instance created successfully")
...
def create_from_input(input_values: Dict[str, Any]) -> ColumnEndPlate:
    """Create an instance of the column column end plate connection module design class from input values."""
...
def generate_output(input_values: Dict[str, Any]):
    ...
    try:
        ...
        raw_output_bolt = module.flange_bolt_spacing(True)
        raw_output_web = module.web_bolt_spacing(True)

        raw_output = raw_output_text + raw_output_bolt + raw_output_web
        ...
        for param in raw_output:
            ...
            key, label, param_type, value = param[:4]
            ...
            elif param_type == "Popup_Section":
                # value = [image_path, width, height, title]
                image_path, width, height, title = value

                # Convert Windows path → Web static path
                image_path = image_path.replace("\\", "/")
                # Extract relative part after /osdag_core/
                rel_path = image_path.split("/osdag_core/")[-1]
                url = f"/static/{rel_path}"

                popup_key = f"Popup.{title.replace(' ', '_')}"
            ...
            elif param_type == "Output_dock_Button":
                continue
...
def create_cad_model(input_values: Dict[str, Any], section: str, session: str) -> str:
    ...
    part_names = ["Column", "Connector", "Model"]
    part_files = {}
    compound_model = None
