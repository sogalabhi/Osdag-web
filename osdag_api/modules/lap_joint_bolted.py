...
def get_required_keys() -> List[str]:
    ...
    print("in lap_joint_bolted.py: get_required_keys called, returning:", req_keys)
    return req_keys

def validate_input(input_values: Dict[str, Any]) -> None:
    print("in lap_joint_bolted.py: validate_input called with input_values:", input_values)
    required_keys = get_required_keys()
    print("in lap_joint_bolted.py: Required keys:", required_keys)
    missing_keys = contains_keys(input_values, required_keys)
    print("in lap_joint_bolted.py: Missing keys:", missing_keys)
    if missing_keys is not None:
        print("in lap_joint_bolted.py: Missing key detected:", missing_keys[0])
        raise MissingKeyError(missing_keys[0])

    # Commented until knowing exact types to validate the keys against.
    # Validate Bolt.Bolt_Hole_Type
    # print("in lap_joint_bolted.py: Validating Bolt.Bolt_Hole_Type")
    # if not isinstance(input_values["Bolt.Bolt_Hole_Type"], str):
    #     print("in lap_joint_bolted.py: Invalid type for Bolt.Bolt_Hole_Type")
    #     raise InvalidInputTypeError("Bolt.Bolt_Hole_Type", "str")

    # # Validate Bolt.Diameter
    # print("in lap_joint_bolted.py: Validating Bolt.Diameter")
    ...
    #     print("in lap_joint_bolted.py: Invalid type for Bolt.Diameter")
    #     raise InvalidInputTypeError("Bolt.Diameter", "non empty List[str] where all items can be converted to int")

    # # Validate Bolt.Grade
    # print("in lap_joint_bolted.py: Validating Bolt.Grade")
    ...
    #     print("in lap_joint_bolted.py: Invalid type for Bolt.Grade")
    ...
    #     print("in lap_joint_bolted.py: Invalid type for Connector.Plate.Thickness_List")
...
def create_module() -> LapJointBolted:
    print("in lap_joint_bolted.py: create_module called")
    module = LapJointBolted()
    print("in lap_joint_bolted.py: LapJointBolted instance created:", module)
    module.set_osdaglogger(None)
    print("in lap_joint_bolted.py: set_osdaglogger called with None")
    return module

def create_from_input(input_values: Dict[str, Any]) -> LapJointBolted:
    try:
        module = create_module()
        print("in lap_joint_bolted.py: Module created successfully")
    except Exception as e:
        print("in lap_joint_bolted.py: Exception in create_module:", e)
        print("in lap_joint_bolted.py: Error in creating module")
    # Now call set_input_values with the updated dictionary
    try:
        module.set_input_values(input_values)
        print("in lap_joint_bolted.py: set_input_values called successfully")
    except Exception as e:
        print("in lap_joint_bolted.py: Exception in set_input_values:", e)
        print("in lap_joint_bolted.py: Error in setting the input values")
    
    return module

def generate_output(input_values: Dict[str, Any]) -> Dict[str, Any]:
    print("in lap_joint_bolted.py: generate_output called with input_values:", input_values)
    output = {}
    module = create_from_input(input_values)
    print("in lap_joint_bolted.py: Module after create_from_input:", module)
    print("in lap_joint_bolted.py: type of module:", type(module))
    raw_output_text = module.output_values(True)
    print("in lap_joint_bolted.py: raw_output_text:", raw_output_text)
    ## Spacing isn't working so commented out for now
    # raw_output_spacing = module.spacing(True)
    # print("in lap_joint_bolted.py: raw_output_spacing:", raw_output_spacing)
    
    # raw_output_capacities = module.capacities(True)
    # print("in lap_joint_bolted.py: raw_output_capacities:", raw_output_capacities)
    # raw_output_bolt_capacity = module.bolt_capacity_details(True)
    # print("in lap_joint_bolted.py: raw_output_bolt_capacity:", raw_output_bolt_capacity)
    logs = module.logs
    print("in lap_joint_bolted.py: logs:", logs)
    raw_output = raw_output_text
    print("in lap_joint_bolted.py: raw_output combined:", raw_output)
    for param in raw_output:
        print("in lap_joint_bolted.py: Processing param:", param)
        if param[2] == "TextBox":
            key = param[0]
            label = param[1]
            value = param[3]
            ...
            print(f"in lap_joint_bolted.py: Added output[{key}] = {output[key]}")
    print("in lap_joint_bolted.py: Final output dict:", output)
    print("in lap_joint_bolted.py: Output keys:", list(output.keys()))
    print("in lap_joint_bolted.py: Returning logs:", logs)
    return output, logs
