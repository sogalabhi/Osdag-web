...
def create_cad_model(input_values: Dict[str, Any], section: str, session: str) -> str:
    if section not in ("Model", "Member", "Plate", "Endplate"):
        print("in welded_tension_member.py: Invalid section:", section)
        raise InvalidInputTypeError("section","Endplate")
    module = create_from_input(input_values)
    print("in welded_tension_member.py: module from input values:", module)
    # print("in welded_tension_member.py: Connectivity:", module.connectivity)
    print("in welded_tension_member.py: module:", module.module)
    print("in welded_tension_member.py: Mainmodule:", module.mainmodule)
    try:
        cld = CommonDesignLogic(None, '', KEY_DISP_TENSION_WELDED, module.mainmodule)
        cld.module_class = module
        print("in welded_tension_member.py: cld.module_class set to:", cld.module_class)
        cld.TObj = cld.createTensionCAD()
        print("in welded_tension_member.py: CommonDesignLogic instance created")
    except Exception as e:
        print("in welded_tension_member.py: error in cld e:", e)
    try:
        tbm.setup_for_cad(cld, module)
        print("in welded_tension_member.py: setup_for_cad called successfully")
    except Exception as e:
        print("in welded_tension_member.py: Error in setting up cad e:", e)
    cld.component = section
    print("in welded_tension_member.py: cld.component set to:", section)
    try:
        model = cld.create2Dcad()
        print("in welded_tension_member.py: create2Dcad called successfully")
    except Exception as e:
        print("in welded_tension_member.py: Error in cld.create2Dcad() e:", e)
        return False
    if not os.path.exists(os.path.join(os.getcwd(), "file_storage/cad_models/")):
        print("in welded_tension_member.py: path does not exist, creating cad_models folder")
        os.mkdir(os.path.join(os.getcwd(), "file_storage/cad_models/"))
    print("in welded_tension_member.py: 2d model:", model)
    file_name = session + "_" + section + ".brep"
    file_path = "file_storage/cad_models/" + file_name
    print("in welded_tension_member.py: brep file path in create_cad_model:", file_path)
    try:
        BRepTools.breptools.Write(model, file_path, Message_ProgressRange())
        print("in welded_tension_member.py: BRepTools.breptools.Write called successfully")
        if section == "Model":
            ...
            if step_writer.Write(full_step_file_path) == 1:
                print(f"in welded_tension_member.py: STEP file saved at {full_step_file_path}")
            else:
                print("in welded_tension_member.py: Warning: Failed to save STEP file!")
            ...
            if iges_writer.Write(full_iges_file_path) == 1:
                print(f"in welded_tension_member.py: IGES file saved at {full_iges_file_path}")
            else:
                print("in welded_tension_member.py: Warning: Failed to save IGES file!")
    except Exception as e:
        print("in welded_tension_member.py: Writing to BREP file failed e:", e)
    return file_path
